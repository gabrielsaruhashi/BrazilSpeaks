...
print("Number of dimensions in our dataset (read ")
dim(music)
```
...
#Get permutation p-value, skip the plot
#magnitude of strength between two continou avriables,. did we seee a samplec orrelation just by chance? 
#dcoukld that happenn just by chance? sample correlation is 1,could we have somtthing as large as thanb y chance? if correlation  is0 , it is arbitrary which value 
# 10000 times, we never saw a correlation by .2 jkust by chance, our actula is .5
...
```
Model seems tohave good fit, no evidence of heteroskedasticity,residuals seem to be approximately normallydistributed, although there is evidence that it is not normal distribution
## Lyric Analysis
